...
Heimgeist manages its own event cursor in `STATE_DIR/chronik.cursor`. `STATE_DIR` is derived from the Heimgeist state-path config (default: `heimgeist_state/` unless overridden).
