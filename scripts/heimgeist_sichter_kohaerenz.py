...
def _meta_sanity(doc: Dict[str, Any]) -> List[Finding]:
    ...
    # Primary expectation: repoLens
    if contract != "repolens-agent" or ver != "v1":
        # Backcompat hint: old wc-merger snapshots might exist
...
def _duplicates_within_repo(files: List[Dict[str, Any]]) -> List[Tuple[str, str]]:
    """
    Returns duplicates of (repo, path) within the same repo.
    Multi-repo snapshots may legitimately repeat the same path across repos.
    """
    seen = set()
    dups = set()
...
def build_report(doc: Dict[str, Any], input_path: Path) -> Report:
    ...
    for r, p in _iter_paths(files):
        rr = r if r in repo_to_paths else (r or "")
...
def render_markdown(rep: Report) -> str:
    ...
    # Verdichtete Essenz + ironischer Stachel
...
def main() -> int:
    ...
    ap.add_argument("--fail-on", default="crit", choices=["none", "warn", "crit"],
                    help="Exit != 0 bei Severity >= Schwelle")
    args = ap.parse_args()
    ...
    # fail-on logic
    maxsev = _max_severity(rep.findings)
    threshold = {"none": 99, "warn": 2, "crit": 3}[args.fail_on]
    if _severity_rank(maxsev) >= threshold:
        # warn => non-zero if threshold hit
